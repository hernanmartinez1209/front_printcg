...
const PrintControls = ({ selectedPrinter, document }) => {
  ...
  const handlePrint = async () => {
    if (!selectedPrinter || !document) return;
    
    setIsPrinting(true);
    setPrintStatus(null);
    
    const result = await printDocument(selectedPrinter.id, document.url);
    ...
    <div className="bg-white shadow sm:rounded-lg">
      <div className="px-4 py-5 sm:p-6">
        ...
        <div className="mt-4">
          <button
            onClick={handlePrint}
            disabled={!selectedPrinter || !document || isPrinting}
            className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${
              !selectedPrinter || !document || isPrinting
